...
ev_batch_size = 128
num_epochs =40
model_name = 'baseline_ego_routine_nn'
...
def plot_metrics(history_path,results_path):
  ...
  model_string = 'Topic Model for k=4 for EgoRoutine'
...
#model.load_weights('/data/s4133366/saved_models/baseline_ego_routine_1/baseline_ego_routine_1_weights/baseline_ego_routine_1_weights.h5')
...
history = model.fit(train_ds,epochs=num_epochs,callbacks=callbacks,verbose=1,workers=8,use_multiprocessing = False,steps_per_epoch = steps,shuffle = False,validation_data = eval_ds,validation_steps = steps_eval)
