...
cache_te = os.path.join(cache_dir,model_name+'_te')
...
test_labels = test_labels.map(inv_map)
...
df_cm.to_csv(os.path.join(pred_dir,'cm_imp1.csv'))

plt.figure(figsize=(30,30))
sn.set(font_scale=1.4) # for label size
sn.heatmap(df_cm, annot=True, annot_kws={"size": 15},cmap="BuPu") # font size
plt.title('Confusion matrix for '+model_string+' using '+db_name+' Dataset with 20 places')
